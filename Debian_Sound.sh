...
if ! command -v inxi &>/dev/null; then
    echo "⚠ inxi не установлен. Установите с помощью: sudo apt install inxi" | tee -a "$LOG_FILE"
else
    inxi -A | tee -a "$LOG_FILE"
fi
...
# 6. Обновление initramfs в Ubuntu
echo -e "\n⏳ Обновление initramfs:" | tee -a "$LOG_FILE"
if command -v update-initramfs &>/dev/null; then
    sudo update-initramfs -u -k all | tee -a "$LOG_FILE"
else
    echo "⚠ Команда update-initramfs не найдена. Это необычно для Ubuntu." | tee -a "$LOG_FILE"
fi
